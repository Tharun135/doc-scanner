...
Now provide your suggestion for the current issue:"""
    ...
    def generate_contextual_suggestion(self, feedback_text: str, sentence_context: str = "",
        ...
        Optionally combines OpenAI with RAG for enhanced context.
        
        Returns:
            Dict containing suggestion, confidence, and metadata
        """
        try:
            # First, try to get RAG-enhanced suggestion if available
            rag_suggestion = None
            if RAG_AVAILABLE and document_content:
                try:
                    rag_suggestion = get_rag_suggestion(
                    ...
                    if rag_suggestion:
                        logger.info("RAG suggestion obtained successfully")
                except Exception as e:
                    logger.warning(f"RAG suggestion failed, continuing with OpenAI: {e}")
            ...
            if ("may" in feedback_lower and "possibility" in feedback_lower) or \
               ("may" in feedback_lower and "permission" in feedback_lower):
                logger.info("Using custom modal verb logic instead of AI")
                fallback_result = self.generate_smart_fallback_suggestion(feedback_text, sentence_context)
                return self._combine_suggestions(fallback_result, rag_suggestion)
            ...
            if ("back up" in feedback_lower or "backup" in feedback_lower) and sentence_context:
                logger.info("Using custom backup/back up logic instead of AI")
                fallback_result = self.generate_smart_fallback_suggestion(feedback_text, sentence_context)
                return self._combine_suggestions(fallback_result, rag_suggestion)
            
            # Check if we have a valid model
            if not self.model_name:
                logger.warning("No AI model available, falling back to rule-based suggestions")
                fallback_result = self.generate_smart_fallback_suggestion(feedback_text, sentence_context)
                return self._combine_suggestions(fallback_result, rag_suggestion)
            ...
            # Combine with RAG suggestion if available
            return self._combine_suggestions(openai_result, rag_suggestion)
            
        except Exception as e:
            logger.error(f"OpenAI ChatGPT suggestion failed: {str(e)}")
            # Fall back to improved rule-based suggestions
            fallback_result = self.generate_smart_fallback_suggestion(feedback_text, sentence_context)
            # Still try to combine with RAG if available
            return self._combine_suggestions(fallback_result, rag_suggestion)
    ...
    def _combine_suggestions(self, primary_result: Dict[str, Any], rag_result: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Combine primary AI suggestion with RAG suggestion for enhanced output.
        """
        if not rag_result:
            return primary_result
        
        try:
            # Extract the main suggestions
            primary_suggestion = primary_result.get("suggestion", "")
            rag_suggestion = rag_result.get("suggestion", "")
            
            # Create combined suggestion
            combined_suggestion = self._format_combined_suggestion(primary_suggestion, rag_suggestion)
            
            # Determine combined confidence
            primary_conf = primary_result.get("confidence", "medium")
            rag_conf = rag_result.get("confidence", "medium")
            combined_confidence = self._combine_confidence(primary_conf, rag_conf)
            
            # Merge context information
            context_used = primary_result.get("context_used", {})
            if "rag_sources" not in context_used:
                context_used["rag_sources"] = rag_result.get("sources", [])
            ...
            logger.warning(f"Failed to combine suggestions: {e}")
            return primary_result
    
    def _format_combined_suggestion(self, primary: str, rag: str) -> str:
        """Format the combined suggestion for display."""
        if not rag:
            return primary
        
        # If RAG suggestion is substantially different, show both
        if self._suggestions_are_different(primary, rag):
            return f"{primary}\n\n📚 ADDITIONAL CONTEXT:\n{rag}"
        else:
            # If similar, enhance the primary with RAG insights
            return self._enhance_primary_with_rag(primary, rag)
    
    def _suggestions_are_different(self, suggestion1: str, suggestion2: str) -> bool:
        """Check if two suggestions provide different advice."""
        # Simple heuristic: if they share less than 50% of words, they're different
    ...
    def _enhance_primary_with_rag(self, primary: str, rag: str) -> str:
        """Enhance primary suggestion with insights from RAG."""
        # Extract any examples or specific guidance from RAG
        rag_lines = rag.split('\n')
        examples = []
        reasons = []
    ...
    def _combine_confidence(self, conf1: str, conf2: str) -> str:
        """Combine confidence levels from two sources."""
        confidence_map = {"low": 1, "medium": 2, "high": 3}
        
        val1 = confidence_map.get(conf1, 2)
        val2 = confidence_map.get(conf2, 2)
...
def get_enhanced_ai_suggestion(feedback_text: str, sentence_context: str = "",
                             ...
                             document_content: str = "") -> Dict[str, Any]:
    """
    Convenience function to get enhanced AI suggestions with optional RAG enhancement.
