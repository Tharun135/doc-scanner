...
const resultsProvider_1 = require("./providers/resultsProvider");
...
function activate(context) {
    ...
    agent = new agent_1.DocumentReviewAgent();
    // Initialize providers
    statusProvider = new statusProvider_1.AgentStatusProvider(agent);
    resultsProvider = new resultsProvider_1.DocumentResultsProvider();
    suggestionsProvider = new suggestionsProvider_1.SuggestionsProvider();
...
async function analyzeDocument(uri) {
    try {
        ...
        if (result.success) {
            ...
            vscode.window.showInformationMessage(`Analysis complete: ${result.data.total_issues} issues found`, 'View Results').then(selection => {
...
async function analyzeSelection() {
    ...
    try {
        ...
        if (result.success) {
            ...
            const panel = vscode.window.createWebviewPanel('selectionAnalysis', 'Selection Analysis', vscode.ViewColumn.Beside, { enableScripts: true });
...
async function batchAnalyze() {
    ...
    const confirmed = await vscode.window.showWarningMessage(`Analyze ${files.length} documents? This may take a while.`, 'Yes', 'No');
    if (confirmed !== 'Yes') {
        return;
    }
    try {
        ...
        if (results.success) {
            ...
            vscode.window.showInformationMessage(`Batch analysis complete: ${results.data.successful_files}/${results.data.total_files} files processed`, 'View Results').then(selection => {
                if (selection === 'View Results') {
                    vscode.commands.executeCommand('document-review-agent.showDashboard');
                }
            });
...
async function showDashboard() {
    ...
    const panel = vscode.window.createWebviewPanel('documentReviewDashboard', 'Document Review Dashboard', vscode.ViewColumn.One, {
        enableScripts: true,
...
// Helper functions
async function applySuggestion(editor, selection, suggestion) {
...
function showSuggestionDetails(suggestion) {
    const panel = vscode.window.createWebviewPanel('suggestionDetails', 'Suggestion Details', vscode.ViewColumn.Beside, { enableScripts: true });
    panel.webview.html = `
        <!DOCTYPE html>
        <html>
        <head>
...
function generateAnalysisHTML(analysis, text) {
    return `
        <!DOCTYPE html>
        <html>
        <head>
...
function generateDashboardHTML(status, results) {
    ...
    return `
        ...
        <body>
            ...
            <div class="results">
                ...
                ${resultEntries.map(([filePath, result]) => `
