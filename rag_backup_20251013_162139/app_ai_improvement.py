...
def get_enhanced_ai_suggestion(
    ...
    """
    Enhanced AI suggestion using the new enhanced RAG system.
    This function has been updated to use the enhanced enrichment system.
    """
    ...
    try:
        # Use our enhanced enrichment system
        from .services.enrichment import enrich_issue_with_solution
        
        # Create issue object if not provided
        if not issue:
            issue = {
        ...
        enhanced_result = enrich_issue_with_solution(issue)
        
        # Convert to expected format
        result = {
            "suggestion": enhanced_result.get("proposed_rewrite", ""),
            "ai_answer": enhanced_result.get("solution_text", ""),
            "confidence": "high" if enhanced_result.get("method", "").startswith("enhanced") else "medium",
        ...
        logger.info(f"✅ Enhanced system result: method={result['method']}")
        return result
