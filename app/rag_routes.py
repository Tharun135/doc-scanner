...
@rag.route('/upload_knowledge')
def upload_knowledge():
    """Upload knowledge page - shows error when RAG system is not available"""
    return render_template('rag/upload_knowledge.html', 
                         rag_available=False,
                         error_message="Document upload is not available. RAG system dependencies are missing.")

@rag.route('/evaluation_dashboard')
def evaluation_dashboard():
    """Evaluation dashboard - shows error when RAG system is not available"""
    return render_template('rag/dashboard.html', 
                         rag_available=False,
                         dependencies_missing=True,
                         error_message="Evaluation dashboard is not available. RAG system dependencies are missing.",
                         stats={
                             'total_chunks': 0,
                             'total_queries': 0,
                             'avg_relevance': 0.0,
