...
# Import RAG modules with better error handling
RAG_AVAILABLE = False
DocumentLoader = None
TextChunker = None
...
try:
    # Test for core dependencies first
    import chromadb
...
def init_rag_system():
    ...
    if not RAG_AVAILABLE or AdvancedRetriever is None:
        logger.warning("⚠️ RAG system not available - install dependencies with: pip install chromadb sentence-transformers")
        RAG_AVAILABLE = False
        return False
    ...
    try:
        ...
        RAG_AVAILABLE = True  # Ensure it's set to True after successful initialization
        logger.info("✅ RAG system initialized successfully")
        return True
    except Exception as e:
        logger.error(f"❌ Failed to initialize RAG system: {e}")
        logger.warning("⚠️ RAG system disabled - some features will be unavailable")
        RAG_AVAILABLE = False  # Set to False if initialization fails
        return False
...
def knowledge_base_dashboard():
    ...
    return render_template('rag/dashboard.html', 
                         stats=stats, 
                         supported_formats=get_supported_formats() if RAG_AVAILABLE else [],
