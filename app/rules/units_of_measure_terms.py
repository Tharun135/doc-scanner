...
def check(content):
    ...
    for match in matches:
        ...
        suggestions.append("Use numerals for numbers with units: '{match.group()}' should be '{numeral} {unit}'.")
    ...
    for match in matches:
        ...
        suggestions.append("Remove the period from unit abbreviations: '{match.group()}' should be '{number} {unit}'.")
    ...
    for match in matches:
        ...
        if plural_s:
            suggestions.append("Use singular form for 1 unit: '{match.group()}' should be '1 {unit}'.")
    ...
    for match in matches:
        number = match.group(1)
        suggestions.append("Remove space between number and '%': '{match.group()}' should be '{number}%'.")
    ...
    for match in matches:
        unit = match.group()
        suggestions.append("Use 'per' instead of '/': '{unit}' should be written out (e.g., 'kilometers per hour').")
    ...
    for match in matches:
        ...
        suggestions.append("Spell out units of time in text: '{match.group()}' should be '{number} {unit_full}'.")
    ...
    for match in matches:
        ...
        suggestions.append("Remove space between number and degree symbol: '{match.group()}' should be '{number}{degree_unit}'.")
    ...
    for measurement, unit_list in measurement_types.items():
        ...
        if len(units_in_document) > 1:
            ...
            suggestions.append("Inconsistent units of {measurement} found: {', '.join(units_in_document)}. Consider using consistent units throughout the document.")
    ...
    for unit in obsolete_units:
        ...
        for match in matches:
            suggestions.append("Consider using SI units instead of '{unit}'.")
    ...
    for match in matches:
        ...
        suggestions.append("Place the currency symbol before the amount: '{match.group()}' should be '{symbol}{number}'.")
